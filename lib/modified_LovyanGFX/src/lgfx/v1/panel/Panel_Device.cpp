...
//----------------------------------------------------------------------------
  ...
  bool Panel_Device::init(bool use_reset)
  {
    ...
    // Serial.printf("Panel_Device::init init_cs()\n");
    init_cs();
    // Serial.printf("Panel_Device::init init_cs() done\n");
    if (_light)
    {
      // Serial.printf("Panel_Device::init _light->init(0)\n");
      _light->init(0);
      // Serial.printf("Panel_Device::init _light->init(0) done\n");
    }
    if (use_reset)
    {
      // Serial.printf("Panel_Device::init rst_control(false)\n");
      rst_control(false);
...
//----------------------------------------------------------------------------
  ...
  void Panel_Device::init_cs(void)
  {
    ...
    lgfx::gpio_hi(pin);
    // Serial.printf("Panel_Device::init_cs(%d) pinMode(output)\n", pin);
  ...
  void Panel_Device::init_rst(void)
  {
    ...
    lgfx::gpio_hi(pin);
    // Serial.printf("Panel_Device::init_rst(%d) pinMode(output)\n", pin);
    lgfx::pinMode(pin, pin_mode_t::output);
  }
  ...
  void Panel_Device::cs_control(bool level)
  {
    ...
    if (level)
    {
      // Serial.printf("Panel_Device::cs_control(%d) gpio_hi\n", pin);
      gpio_hi(pin);
    }
    else
    {
      // Serial.printf("Panel_Device::cs_control(%d) gpio_lo\n", pin);
      gpio_lo(pin);
    }
  }
  ...
  void Panel_Device::rst_control(bool level)
  {
    ...
    if (level)
    {
      // Serial.printf("Panel_Device::rst_control(%d) gpio_hi\n", pin);
      gpio_hi(pin);
    }
    else
    {
      // Serial.printf("Panel_Device::rst_control(%d) gpio_lo\n", pin);
      gpio_lo(pin);
    }
  }
