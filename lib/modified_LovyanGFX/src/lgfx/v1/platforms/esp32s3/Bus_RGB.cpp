...
// フレームバッファの確保やイベントハンドラは自前で処理したいので、敢えて i80バスを作成する。
    ...
    // Serial.println("set clock source: " + String(_panel_config.clk_src));
    // _panel_config.timings.pclk_hz = _cfg.freq_write;
    // Serial.println("set pixel clock: " + String(_panel_config.timings.pclk_hz));
    // Serial.println("panel width: " + String(_cfg.panel->width()));
    // Serial.println("panel height: " + String(_cfg.panel->height()));
    // // _panel_config.timings.h_res = 1;//_cfg.panel->width();
    // _panel_config.timings.h_res = _cfg.panel->width();
    // Serial.println("panel h_res: " + String(_panel_config.timings.h_res));
    // // _panel_config.timings.v_res = 1;//_cfg.panel->height();
    // _panel_config.timings.v_res = _cfg.panel->height();
    // Serial.println("panel v_res: " + String(_panel_config.timings.v_res));
    ...
    // Serial.println("   set data pins: ");
    for (int i = 0; i < 16; ++i)
    {
      bus_config.data_gpio_nums[i^8] = _cfg.pin_data[i];
    }
    // Serial.println("   set bus width: ");
    bus_config.bus_width = 16;
    
    // Serial.println("   set psram transfer alignment: ");
