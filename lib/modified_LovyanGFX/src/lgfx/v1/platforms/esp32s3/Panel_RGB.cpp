...
namespace lgfx
{
  inline namespace v1
  {
    ...
    */
    ...
    {
      // OFFIZIELLE HERSTELLERSEQUENZ, ÜBERSETZT FÜR LOVYANGFX
      static constexpr const uint8_t list0[] =
      {
        0x11, CMD_INIT_DELAY, 120, // Sleep Out & Warten
        ...
        0x82, 2, 0x00, 0x00,
        0x80, 1, 0x4a,
        0xB1, 1, 0x30, // KRITISCH: BGR Mode, Column Inversion, Normale Scanrichtung
        0x7A, 2, 0x0F, 0x13,
        0x7B, 2, 0x0F, 0x13,
        0x6D, 32, 0x03, 0x03, 0x1e, 0x02, 0x08, 0x1a, 0x19, 0x0d, 0x11, 0x12, 0x13, 0x14, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x0c, 0x0b, 0x0a, 0x09, 0x0d, 0x19, 0x1a, 0x07, 0x01, 0x1E, 0x03, 0x03,
        0x64, 16, 0x38, 0x04, 0x03, 0xc4, 0x03, 0x03, 0x38, 0x02, 0x03, 0xc6, 0x03, 0x03, 0x2C, 0x7A, 0x2C, 0x7A,
        0x65, 16, 0x38, 0x08, 0x03, 0xc0, 0x03, 0x03, 0x38, 0x06, 0x03, 0xc2, 0x03, 0x03, 0x2C, 0x7A, 0x2C, 0x7A,
        0x66, 16, 0x83, 0xd0, 0x03, 0xc4, 0x03, 0x03, 0x83, 0xd0, 0x03, 0xc4, 0x03, 0x03, 0x2C, 0x7A, 0x2C, 0x7A,
        0x60, 8, 0x38, 0x0c, 0x3c, 0x3c, 0x38, 0x0b, 0x3c, 0x3c,
        0x61, 8, 0xb3, 0xc4, 0x3c, 0x3c, 0xb3, 0xc4, 0x3c, 0x3c,
        0x62, 8, 0xb3, 0xc4, 0x3c, 0x3c, 0xb3, 0xc4, 0x3c, 0x3c,
        0x63, 8, 0x38, 0x0a, 0x3c, 0x3c, 0x38, 0x09, 0x3c, 0x3c,
        0x68, 13, 0x77, 0x08, 0x0a, 0x08, 0x09, 0x00, 0x00, 0x18, 0x0a, 0x08, 0x09, 0x00, 0x00,
        ...
        // Fügen Sie den fehlenden Pixelformat-Befehl hinzu!
        0x3A, 1, 0x55,                   // Pixel Format Set (RGB565)
        // 0x3A, 1, 0x77,                   // Pixel Format Set (RGB888)

        0x29, CMD_INIT_DELAY, 20,       // Display ON
        0xFF, 0xFF,                     // End of sequence
      };
      // {
      ...
      //   0x11, CMD_INIT_DELAY, 120,              // Sleep Out, then 120ms delay
      //   // 0x3A, 1, 0x50,                          // Pixel Format Set (0x50 = RGB565)
      //   0x3A, 1, 0x55,                          // Pixel Format Set (0x55 = RGB565 and MIDI DSI RGB565)
      //   // 0x3A, 1, 0x60,                          // Pixel Format Set (0x60 = RGB666)
      //   // 0x3A, 1, 0x70,                          // Pixel Format Set (0x70 = RGB888)
      //   0x29, 0,                               // Display ON
      //   0xFF, 0xFF,                            // End of sequence
    ...
    bool Panel_RGB::sendCommand(uint8_t cmd, const uint8_t* data, size_t len, uint32_t post_delay)
    {
      int32_t pin_mosi = _config_detail.pin_mosi;
      int32_t pin_sclk = _config_detail.pin_sclk;
      int32_t pin_cs   = _config_detail.pin_cs;
      if (pin_mosi < 0 || pin_sclk < 0 || pin_cs < 0) {
        Serial.println("Panel_RGB::sendCommand aborted (missing pins)");
        return false;
      }
      // Backup and set required pin modes (cheap since rarely used)
      lgfx::gpio::pin_backup_t backup_pins[] = {(gpio_num_t)pin_mosi, (gpio_num_t)pin_sclk, (gpio_num_t)pin_cs};
      lgfx::gpio_lo(pin_mosi); lgfx::pinMode(pin_mosi, pin_mode_t::output);
      lgfx::gpio_lo(pin_sclk); lgfx::pinMode(pin_sclk, pin_mode_t::output);
      lgfx::gpio_lo(pin_cs);   lgfx::pinMode(pin_cs,   pin_mode_t::output);

      // Command phase
      _write_swspi(cmd & 0xFF, 9, pin_sclk, pin_mosi);
      // Data phase
      for (size_t i = 0; i < len; ++i) {
        _write_swspi((data[i] | 0x100), 9, pin_sclk, pin_mosi);
      }
      lgfx::gpio_hi(pin_cs);
      if (post_delay) delay(post_delay);
      for (auto &b : backup_pins) { b.restore(); }
      return true;
    }

    void Panel_RGB::displayOn()  { sendCommand(0x29, nullptr, 0, 0); }
    void Panel_RGB::displayOff() { sendCommand(0x28, nullptr, 0, 0); }
    void Panel_RGB::sleepIn()    { sendCommand(0x10, nullptr, 0, 120); }
    void Panel_RGB::sleepOut()   { sendCommand(0x11, nullptr, 0, 120); }
