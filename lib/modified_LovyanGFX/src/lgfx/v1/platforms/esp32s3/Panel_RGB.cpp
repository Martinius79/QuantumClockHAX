    ...
    */
      ...
      static constexpr const uint8_t list0[] =
          {
            ...
            0xC1, 1, 0x3F,                         // Display Control
            0xCD, 1, 0x25,                         // Source Driver Control
            0xC9, 1, 0x10,                         // Source Driver Control
            0xF8, 1, 0x8A,                         // Power Control
            0xAC, 1, 0x45,                         // VCOM Setting (Spannung)
            0xA7, 1, 0x47,                         // Power Control
            0xA0, 1, 0xCC,                         // CHP_CTL5 (Charge Pump, beeinflusst Spannung)
            0x86, 4, 0x99, 0xA3, 0xA3, 0x31,       // CHP_CTL6 (Charge Pump, beeinflusst Spannung)
            ...
            0x9A, 1, 0x4A,                         // Unbekannt, vermutlich Power/Timing
            0x9B, 1, 0x22,                         // Unbekannt, vermutlich Power/Timing

            0x82, 2, 0x00, 0x00,                   // Unbekannt, vermutlich Power/Timing
            0x80, 1, 0x54,                         // Unbekannt, vermutlich Power/Timing
            0xB1, 1, 0x33,                         // Display Function Control
            // 0x33 = 0b00110011
            // Bit 5 (1): Gate scan direction (GS) - z.B. Scan order, Mirror/Flip
            // Bit 4 (1): Source scan direction (SS) - z.B. Scan order, Mirror/Flip
            // Bit 1 (1): Display output mode (z.B. RGB interface enable)
            // Bit 0 (1): Display output mode (z.B. DE mode enable)
            // Bits 7,6,3,2 (0): Reserviert oder nicht genutzt
            // Siehe: ILI9488/GC9A01/ST7701 Datenblätter, Open-Source-Treiber

            0x7A, 2, 0x0F, 0x13,                   // Unbekannt, vermutlich Gamma/Timing
            0x7B, 2, 0x0F, 0x13,                   // Unbekannt, vermutlich Gamma/Timing

            0x6D, 32, 0x0C, 0x03, 0x1E, 0x02, 0x08, 0x1A, 0x19, 0x03, 0x0D, 0x0E, 0x0F, 0x10, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x1E, 0x11, 0x12, 0x13, 0x14, 0x03, 0x19, 0x1A, 0x07, 0x01, 0x1E, 0x03, 0x0C, // Gamma-Kurve/Look-Up Table
            // 0x6D: Gamma Correction Table (32 Werte, meist für positive/negative Farbverläufe, beeinflusst Farbdarstellung)
            0x64, 16, 0x38, 0x04, 0x03, 0xC4, 0x03, 0x03, 0x38, 0x02, 0x03, 0xC6, 0x03, 0x03, 0x2C, 0x7A, 0x2C, 0x7A, // Gamma/Timing
            // 0x64: Gamma/Timing Register (16 Werte)
            //  - Typisch für GC9503CV: Feineinstellung der Gamma-Kurve, meist für einzelne Farbkanäle (R, G, B) oder Graustufen
            //  - Werte beeinflussen die Spannungslevel für die Darstellung von Farben und Übergängen
            //  - Die genaue Bedeutung der Bytes ist herstellerspezifisch, aber oft: [P1, P2, ..., Pn] für verschiedene Segmente der Gamma-Kennlinie
            //  - Siehe: GC9A01/ILI9488/ST7701 Datenblätter, Open-Source-Treiber
            ...
            // 0x60: Gamma/Timing Register (8 Werte)
            //  - Dient der weiteren Feineinstellung der Gamma-Kurve, meist für bestimmte Segmente oder Übergänge
            //  - Werte beeinflussen die Spannungslevel für Helligkeit und Kontrast in bestimmten Bereichen der Graustufen- oder Farbdarstellung
            //  - Die genaue Bedeutung der Bytes ist herstellerspezifisch, aber oft: [S1, S2, ..., S8] für Segmentpunkte der Gamma-Kennlinie
            //  - Siehe: GC9A01/ILI9488/ST7701 Datenblätter, Open-Source-Treiber
            ...
            // 0xD1: Gamma/Timing Table (52 Werte)
            //  - Enthält die Gamma-Kennlinie für das Display (z.B. Helligkeits- und Kontrastverlauf)
            //  - Jeder Wert beeinflusst die Spannungsstufen für die einzelnen Graustufen
            //  - Typisch: Wertepaare für verschiedene Farbkanäle oder Graustufen
            //  - Die genaue Zuordnung ist herstellerspezifisch, aber oft:
            //    [V0, V1, V2, ..., Vn] für die Spannungslevel der internen DACs
            //  - Siehe: GC9A01/ILI9488/ST7701 Datenblätter, Open-Source-Treiber
            ...
            0x11, CMD_INIT_DELAY, 200,              // Sleep Out, dann 200ms Delay
            0x3A, 1, 0x50,                          // Pixel Format Set (0x50 = RGB565)
            0x29, 0,                               // Display ON
            0xFF, 0xFF,                            // Ende der Sequenz
          };
      switch (listno)
      {
      case 0:
    ...
    bool Panel_GC9503::init(bool use_reset)
    {
      ...
      if (pin_mosi >= 0 && pin_sclk >= 0)
      {
        ...
        //Serial.printf("Panel_GC9503 writing initial commands\n");
        //delay(10); // Wait for pin state to stabilize
        for (uint8_t i = 0; auto cmds = getInitCommands(i); i++)
        {
          command_list(cmds);
    ...
    const uint8_t *Panel_GC9503_Quantum_Clock::getInitCommands(uint8_t listno) const
    {
      static constexpr const uint8_t list0[] = 
      {
        0xF0, 5, 0x55, 0xAA, 0x52, 0x08, 0x00, // Command Page Select (Page 0)
        0xF6, 2, 0x5A, 0x87,                   // Interface/Power Control
        0xC1, 1, 0x3F,                         // Display Control (Source Output, Gate Output, Scan Direction aso)
        0xCD, 1, 0x25,                         // Source Driver Control
        0xC9, 1, 0x10,                         // Source Driver Control
        0xF8, 1, 0x8A,                         // Power Control
        0xAC, 1, 0x45,                         // VCOM Setting (Spannung)
        ...
        // 0x33 = 0b 00110011 -> BGR-Mode, Scanrichtung invertiert
        //      Bits 01234567
        // 0x10 = 0b 00010000 -> RGB-Mode, Scanrichtung normal
        // 0x13 = 0b 00010011 -> RGB-Mode, Scanrichtung invertiert
        // Bit 7 (1): SS - Source scan direction (1 = reverse, 0 = normal)
        // Bit 6 (1): GS - Gate scan direction (1 = reverse, 0 = normal)
        // Bit 3-5 (3)- Inersion Mode for Source Driver
    ...
    {
      Serial.printf("Panel_GC9503_Quantum_Clock init\n");
      //delay(10); // Wait for pin state to stabilize
      if (!Panel_RGB::init(use_reset))
      {
      ...
      if (pin_mosi >= 0 && pin_sclk >= 0)
      {
        ...
        // Serial.printf("Panel_GC9503_Quantum_Clock writing initial commands\n");
        // delay(10); // Wait for pin state to stabilize
        for (uint8_t i = 0; auto cmds = getInitCommands(i); i++)
        {
          command_list(cmds);
