...
//ESP_LOGI("LGFX","Bus_SPI::config  spi_port:%d  dc:%0d %02x", spi_port, _cfg.pin_dc, _mask_reg_dc);
  ...
  {
    Serial.printf("Bus_SPI::init()\n");
ESP_LOGI("LGFX","Bus_SPI::init");
    Serial.printf("Bus_SPI::init() dc_control(true)\n");
    dc_control(true);
    Serial.printf("Bus_SPI::init() pinMode(_cfg.pin_dc, pin_mode_t::output)\n");
 ...
 #endif
#endif
    Serial.printf("Bus_SPI::init() spi::init(_cfg.spi_host, _cfg.pin_sclk, _cfg.pin_miso, _cfg.pin_mosi, dma_ch)\n");
    _inited = spi::init(_cfg.spi_host, _cfg.pin_sclk, _cfg.pin_miso, _cfg.pin_mosi, dma_ch).has_value();
    if (!_inited)
    {
      Serial.printf("Bus_SPI::init() spi::init failed\n");     
    }
    else
    {
      Serial.printf("Bus_SPI::init() spi::init success\n");
    }
...
//ESP_LOGI("LGFX","writeCmd: %02x  len:%d   dc:%02x", data, bit_length, _mask_reg_dc);
  ...
  void Bus_SPI::writeData(uint32_t data, uint_fast8_t bit_length)
  {
    ...
    // Serial.printf("Bus_SPI::writeData data: %08x, bit_length: %d\n", data, bit_length);
